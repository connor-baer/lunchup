...
function startBot(teamId, botToken) {
  ...
  rtm.on(RTM_EVENTS.MESSAGE, message => {
    ...
    const groupMembers = api.groups.info(channel, (err, res) => {
      if (err) {
        logger.error(err);
      }
    ...
    if (
      !text ||
      (!contains(text, [`<@${rtm.activeUserId}>`]) && groupMembers.length > 2)
    ) {
      return;
    }
    ...
    switch (true) {
      ...
      case contains(text, [
        ...
        api.chat.postMessage(
          channel,
          'LunchUp matches up two random coworkers every week to go on a blind lunch.',
          {
